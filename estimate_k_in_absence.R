...
paste(pic_str,coef(result)[1],coef(result)[2],coef(result)[3],END_CO2,START_CO2,as.numeric(as.duration(interval(df1[1,1],df1[RANGE,1])),"seconds"),sep=",")
sink()
plot(X,Y1,type="l",col=cols[1],xlim=c(1,X[RANGE]),ylim=c(END_CO2 - 20,START_CO2 + 30),xlab="経過時間[s]",ylab="co2[ppm]")
par(new=T)
plot(X,predict(result),type='l',col=cols[2],xlim=c(1,X[RANGE]),ylim=c(END_CO2 - 20,START_CO2 + 30),xlab="",ylab="")
legend("topright",legend = c("測定データ","推定した値を代入した式"),col = cols,lty = c(1,1))
graphics.off()